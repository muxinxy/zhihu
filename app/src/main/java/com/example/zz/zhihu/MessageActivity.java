...
public class MessageActivity extends AppCompatActivity {
    ...
    private void parseJSONWithJSONObject(String data) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                sendRequestWithHttpURLConnection_Commits();
                messageList.add(new Messages(username_intent,title,id,display_date,images,LCN,SCN));
            }
    ...
    private void sendRequestWithHttpURLConnection_Commits() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                HttpURLConnection connection = null;
                BufferedReader reader = null;
                try {
                    URL url= new URL("https://news-at.zhihu.com/api/4/story-extra/"+id);
                    connection = (HttpURLConnection) url.openConnection();
                    connection.setRequestMethod("GET");
                    connection.setConnectTimeout(8000);
                    connection.setReadTimeout(8000);
                    ...
                    parseJSONWithJSONObject_Commits(response.toString());
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    if (reader != null) {
    ...
    private void parseJSONWithJSONObject_Commits(String data) {
        try {
            JSONObject jsonObject=new JSONObject(data);
            LCN=jsonObject.getString("long_comments");
            SCN=jsonObject.getString("short_comments");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
